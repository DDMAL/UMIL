...
test.describe('Create Instrument - Desktop', () => {
  authTest(
...
test.describe('Delete Instrument - Desktop', () => {
  authTest(
  ...
  test('should not show delete button when not authenticated', async ({
    ...
    // Test that delete button doesn't appear without authentication
    // We use a Wikidata instrument but the test is about auth, not deletability
    await instrumentDetailPage.goto(TEST_INSTRUMENT);

    // Verify page loaded but delete button is not present
