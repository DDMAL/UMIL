...
def add_info_to_wikidata_entity(action, access_token, wikidata_id, value, language):
    ...
    """
    # hardcoded to Wikidata Sandbox for testing
...
def edit_wikidata(request):
    ...
    for instrument_name in approved:
        print(f"Processing instrument name: {instrument_name.name} in language {instrument_name.language.wikidata_code}", flush=True)
        # post to wikidata
